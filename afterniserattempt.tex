...
\begin{frame}{How to Model Real-Life Scenarios}
    ...
    \begin{itemize}
        ...
        \begin{tikzpicture}
            ...
            \node[rectangle,draw] (s) at (0,0) {Scenario};

            \node[align = center, rectangle, draw] (m) at (4,0) {Model\\ (Graph, LP)};
        ...
        \begin{tikzpicture}
            ...
            \node[align = center, rectangle, draw, red] (e) at (8,0) {\textbf{An Event changes}\\ \textbf{scenario}};

            \draw[->] (s) edge (m);


            \draw[->] (e) edge[bend left, red] (s);

            \draw[->] (s) edge[bend left, red] node[above] {REMODEL} (m);
    ...
    \begin{itemize}
            ...
            \begin{figure}[scale = 0.5]
                \begin{tikzpicture}[scale = 0.6]
                    \node[rectangle, draw, align = center] (o) at (4,0) {$G: S\rightarrow aS$\\ $A\rightarrow bS$\\ \vdots\\$B\rightarrow b$};

                    \node (p) at (0, 0) {\includegraphics[scale = 0.2]{images/script.png}};

                    \node (t) at (8, 1) {\huge\textcolor{green}{$\checkmark$}};

                    \node (f) at (8, -1){\huge\textcolor{red}{$\times$}};
            ...
            % \item <4-> \textcolor{red}{Timed Automata}: Does an automated program written for railway gates keep the gates closed even AFTER the train leaves?
...
\begin{frame}{Modelling Knowledge}
    ...
    \setlength\itemsep{1em}
        ...
        \begin{itemize}
        \setlength\itemsep{1em}
            % \item<4-> \textbf{Considering possibilities}: Say $B -> $\textcolor{green}{$2$}, $B$ knows: \txtimg{-2.5mm}{0.1}{images/card-possibleBoC1.png} OR \raisebox{-2.5mm}{\includegraphics[scale= 0.1]{images/card-possibleBoC3.png}}

            \item<4-> \textbf{Considering possibilities}: 
            
            
            \vspace{1em}
            \begin{tikzpicture}[thick,scale=0.7, every node/.style={scale=0.7}]
                \node[draw, thin, rectangle, align = center] (s) at (0, 0){$A > 3$\\ $B > 2$\\ $C > 1$};

                \node[draw, thin, rectangle, align = center] (s) at (2, 0){$A > 1$\\ $B > 2$\\ $C > 3$};

                \node[draw, thin, rectangle, align = center] (s) at (4, 0){$A > 2$\\ $B > 3$\\ $C > 1$};

                \node (etc) at (6, 0){\ldots};
            \end{tikzpicture}
        

            \item<5-> \textbf{Indistinguishable Possibilities}: 

            \vspace{1em}
            For $B$: \begin{tikzpicture}[thick,scale=0.7, every node/.style={scale=0.7}]
                \node[draw, very thick, rectangle, align = center] (a3) at (0, 0){$A > 3$\\ \textcolor{green}{$B > 2$}\\ $C > 1$};

                \node[draw, thin, rectangle, align = center] (a1) at (2, 0){$A > 1$\\ \textcolor{green}{$B > 2$}\\ $C > 3$};

                \draw[-] (a3) edge node[above] {$B$} (a1);
            \end{tikzpicture}, 
            For $A$: \begin{tikzpicture}[thick,scale=0.7, every node/.style={scale=0.7}]
                \node[draw, very thick, rectangle, align = center] (a3) at (0, 0){\textcolor{green}{$A > 3$}\\ $B > 2$\\ $C > 1$};

                \node[draw, thin, rectangle, align = center] (a1) at (2, 0){\textcolor{green}{$A > 3$}\\ $B > 1$\\ $C > 2$};

                \draw[-] (a3) edge node[above] {$A$} (a1);
            \end{tikzpicture}

            \item<6-> \textbf{Event changes Knowledge}: $A$ tells $B$ it has $3$, now for $B$: \begin{tikzpicture}[thick,scale=0.7, every node/.style={scale=0.7}]
                \node[draw, very thick, rectangle, align = center] (s) at (2, 0){$A > 3$\\ \textcolor{green}{$B > 2$}\\ $C > 1$};
            \end{tikzpicture}
        \end{itemize}
    \end{itemize}
\end{frame}

\begin{frame}{Checking Solution vs Finding a Solution}     
    \begin{itemize}
    \setlength\itemsep{1em}
        \item<1-> Two kind of questions using Linear Programs:
        \begin{itemize}
        \setlength\itemsep{1em}
            \item<2-> \raisebox{-10mm}{\begin{tikzpicture}[thick,scale=0.7, every node/.style={scale=0.7}]
            \node [draw, rectangle, align = center] (rect) at (4, 0) {Feasibility\\ Check};
            % \node(M)[rectNode, left of=rect, xshift = -3cm]{~};
            \node(s)[rectNode, align = center] at (0,1){Max $c^Tx$ \\ s.t. $Ax\leq b$};
            \node(f)[rectNode] at (0,-1){$x = \begin{bmatrix} 2\\ 7\\ \vdots\\ 3\end{bmatrix}$};
            \node(op)[rectNode, right of=rect, xshift = 3cm]{Feasible?};
        ...
        \end{tikzpicture}}
            ...
            \item<3-> \raisebox{0mm}{\begin{tikzpicture}[thick,scale=0.7, every node/.style={scale=0.7}]
            \node [draw, rectangle, align = center] (rect) at (4, 0) {Feasible Solution\\ Finding};
            % \node(M)[rectNode, left of=rect, xshift = -3cm]{~};
            \node(s)[rectNode, align = center] at (0,0){Max $c^Tx$ \\ s.t. $Ax\leq b$};
            % \node(f)[rectNode] at (0,-1){$x = \begin{bmatrix} 2\\ 7\\ \vdots\\ 3\end{bmatrix}$};
            \node(op)[rectNode, right of=rect, xshift = 3cm]{$\exists~?~$ a feasible solution};
            \draw[->](s.east)--node[anchor=south] {}(rect.west);
            % \draw[->](M.east)--node[anchor=south] {}(rect.west);
            % \draw[->](f.east)--node[anchor=south] {}(rect.west);
            \draw[->](rect.east)--node[anchor=south] {}(op.west);
...
%      \item<1-> Two Questions involving OS programs:
%         \begin{itemize}
%         \setlength\itemsep{1em}
...
% \begin{frame}{Checking Solution vs Finding a Solution}     
%     Consider these questions:\pause
%     \begin{itemize}
%     \setlength\itemsep{1em}
...
\begin{frame}{Recall: Checking Solution vs Finding a Solution}     
    \begin{itemize}
    \setlength\itemsep{1em}
        \item<1-> Recall the questions using Linear Programs:
        \begin{itemize}
        \setlength\itemsep{1em}
            \item<2-> \raisebox{-10mm}{\begin{tikzpicture}[thick,scale=0.6, every node/.style={scale=0.6}]
            \node [draw, rectangle, align = center] (rect) at (4, 0) {Feasibility\\ Check};
            % \node(M)[rectNode, left of=rect, xshift = -3cm]{~};
            \node(s)[rectNode, align = center] at (0,1){Max $c^Tx$ \\ s.t. $Ax\leq b$};
            \node(f)[rectNode] at (0,-1){$x = \begin{bmatrix} 2\\ 7\\ \vdots\\ 3\end{bmatrix}$};
            ...
            \draw[->](f.east)--node[anchor=south] {}(rect.west);
            \draw[->](rect.east)--node[anchor=south] {}(op.west);
        \end{tikzpicture}} : \textcolor{red}{Check CNF Assignment}

            \item<3-> \raisebox{0mm}{\begin{tikzpicture}[thick,scale=0.6, every node/.style={scale=0.6}]
            \node [draw, rectangle, align = center] (rect) at (4, 0) {Feasible Solution\\ Finding};
            % \node(M)[rectNode, left of=rect, xshift = -3cm]{~};
            \node(s)[rectNode, align = center] at (0,0){Max $c^Tx$ \\ s.t. $Ax\leq b$};
            % \node(f)[rectNode] at (0,-1){$x = \begin{bmatrix} 2\\ 7\\ \vdots\\ 3\end{bmatrix}$};
            ...
            % \draw[->](f.east)--node[anchor=south] {}(rect.west);
            \draw[->](rect.east)--node[anchor=south] {}(op.west);
        \end{tikzpicture}} : \textcolor{red}{CNF-SAT}
        \end{itemize}
...
\begin{frame}{POL Satisfiability}
    ...
    ~~~~~~~~~~~~~~~If \textcolor{red}{NO MODEL} then \textcolor{red}{$\varphi$} is a property
...
\newcommand{\DEXPSPACE}{\mathsf{DOUBLE-EXPSPACE}}
\begin{frame}{$\POL$ Satisfiability Results}
    ...
    \textbf{Yes}.
    \begin{block}{$\POL$- Satisfiability Complexity [To be Submitted]}
        The satisfiability problem of $\POL$ is in $\DEXPSPACE$.
    \end{block}\pause
    \textbf{Open}: Lower Bound
\end{frame}
...
\begin{frame}{$\POL$ Fragment Results (KR'23)\footnotemark[1]}
    \begin{center}
        ...
        \end{tabular}
    \end{center}
    \footnotetext[1]{\tiny Chakraborty S.; Ghosh A.; Ghosh S.; and Schwarzentruber F. 2023. On simple expectations and observations of intelligent agents: A complexity study., KR 2023, Rhodes, Greece, September 2-8, 2023, 136–145.}
\end{frame}
...
\section{Decidability: A High Level Idea}
    ...
    % \begin{frame}{Decidability: Model-Checking}
    %      \textbf{A closer look at residue}: $\LL(\pi\regdiv a) = \{w\mid aw\in\LL(\pi)\}$\pause

    %      \begin{tikzpicture}
    %          \node[draw, circle, align = center] (q0) at (0,0){$q_0$};
    %          \node[draw, circle, align = center] (q1) at (2,0){$q_1$};
    %          \node[draw, circle, align = center] (q2) at (4,0){$q_2$};
    %          \node[draw, circle, align = center, minimum size = 1cm] (q2) at (4,0){};
    %          \node[draw, circle, align = center] (q3) at (2,-1){$q_3$};
    %          \node (e) at (-1,0){};
             
    %          \draw[->](e) edge[red] (q0);
    %          \draw[->](q0) edge node[above] {$a$}(q1);
    %          \draw[->](q0) edge[bend left] node[above] {$a$}(q2);
    %          \draw[->](q1) edge node[above] {$b$}(q2);
    %          \draw[->](q0) edge node[above] {$b$}(q3);
    %          \draw[->](q3) edge node[above] {$b$}(q2);
    %          \draw[->](q3) edge[bend left] node[above]{$a$}(q0);
    %          \draw[->](q2) edge[bend left] node[above]{$a$}(q3);


    %          \node (res) at (5,0){\huge$\regdiv$};
    %          \node (a) at (5.5,0){\huge$a$};
    ...
    %     \textcolor{red}{$\pi$ can have FINITELY many unique residue over $\Sigma^\star$}
    % \end{frame}
...
\begin{frame}{Decidability of $\POL^-$: Some Tableau Rules}
    \begin{figure}[t!]
	\scalebox{0.9}{
\begin{tabular}{lc}
